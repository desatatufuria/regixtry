...
| Interactive setup flow | `regixtry setup` still prompts for `binary-only` vs `daemon-sqlite`; choosing `daemon-sqlite` now also prompts for runtime TLS mode, then prompts for editable `Listen address` and `Public URL`, asks whether auth should be enabled, and when auth is enabled collects the Postgres DSN plus bootstrap admin credentials. It asks for local cert/key paths only in `direct-tls` mode unless those values were already supplied explicitly. |
