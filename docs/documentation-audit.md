...
- Signing policy administration: `GET`/`PUT /admin/v1/signing-policy` (multiple trusted public keys and/or trusted keyless/Fulcio identities per policy, globally and per repository) and `GET /admin/v1/signing-policy/key-usage`, with a fail-closed pull gate when a repository's policy requires a valid signature and none is found. Keyless verification is offline against a pinned trusted root, with no live Rekor query, no operator-supplied root override, and no TUF auto-update (`internal/domain/signing/keyless.go`).
