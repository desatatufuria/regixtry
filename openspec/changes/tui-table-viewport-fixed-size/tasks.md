...
**Remediation note (post sdd-verify FAIL, CRITICAL-01):** `sdd-verify` found the spec.md "Long table pages internally" scenario (Req: Internal Table and List Scrolling) had zero covering tests — the only prior admin-table arrow-key test used 2 rows, never crossing a page boundary. Added `TestModelAdminScanRunsTablePagesOnArrowKeyNavigationPastPageBoundary` (`internal/tui/model_test.go`): a 30-row ScanRuns table at the 90x24 minimum viewport, navigated past the first page boundary via real "down" key presses, asserting `CurrentPage()` advances, the highlighted row is visible in `table.View()`, the first-page-only row scrolls off, the position indicator updates, and screen chrome (title/help/table header/total height) stays unchanged. No production code changed — the underlying mechanism (design.md decision #5, `WithHighlightedRow` auto-paging) was already correct; this closes a test-coverage gap only.
