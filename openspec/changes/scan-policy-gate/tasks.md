...
- [ ] 1.1 Add `ScanPolicySettings` struct and `ScanPolicyThresholdCritical`/
...
- [ ] 1.2 RED `internal/infra/metadata/sqlite/store_test.go`: seed no row, call
...
- [ ] 1.3 RED `store_test.go`: `UpsertScanPolicySettings` then
...
- [ ] 1.4 GREEN: add `scan_policy_settings` `CREATE TABLE IF NOT EXISTS` to
      ...
      `time.RFC3339Nano`.
- [ ] 1.5 Confirm 1.2–1.3 GREEN:
      `go test ./internal/infra/metadata/sqlite/... -run ScanPolicySettings`.
- [ ] 1.6 RED `internal/app/regixtry/service_test.go`:
...
- [ ] 1.7 GREEN: add `Service.GetScanPolicySettings` (code-level default
...
- [ ] 1.8 Confirm 1.6 GREEN: `go test ./internal/app/regixtry/... -run
      ScanPolicySettings`.

## Phase 2: `GetLatestScanRunByDigest` (Decision 2a) — independent of Phase 1

- [ ] 2.1 RED `internal/infra/metadata/sqlite/store_test.go`: seed a `queued`,
...
- [ ] 2.2 RED `store_test.go`: no run ever created for the digest —
      `GetLatestScanRunByDigest` returns typed `domain.ErrorCodeNotFound`, not
      a zero-value `ports.ScanRun`.
- [ ] 2.3 GREEN: add `GetLatestScanRunByDigest` to `store.go` — same SELECT
...
- [ ] 2.4 Confirm 2.1–2.2 GREEN: `go test
...
- [ ] 3.1 RED `internal/app/regixtry/service_scanning_test.go` (new file):
...
- [ ] 3.2 GREEN: implement `scanPolicyViolated(settings ports.ScanPolicySettings,
...
- [ ] 4.6 GREEN: add `Service.enforceScanPolicy(ctx, repository, digest string)
      ...
      `ResolveManifest`.
- [ ] 4.7 RED `internal/protocol/http/router_test.go`: `writeError` maps
      `ErrorCodePolicyViolation` to HTTP 403 with code `DENIED` and **no**
      `WWW-Authenticate` header.
- [ ] 4.8 GREEN: add the `ErrorCodePolicyViolation` case to `writeError`
      (`router.go:559-578`), between `NotFound` and the 400 group.
- [ ] 4.9 Confirm 4.1, 4.3–4.5, 4.7 GREEN: `go test ./internal/app/regixtry/...
...
- [ ] 5.1 Add `ScanTriggerPush = "push"` constant to `internal/ports/regixtry.go`
      (sibling of existing `ScanTrigger*` constants; no migration needed,
      `trigger` column has no CHECK constraint).
- [ ] 5.2 RED `internal/app/regixtry/service_test.go`: `queuePushScan` called
...
- [ ] 5.3 RED `service_test.go`: `queuePushScan` creates nothing when
      `ScanSettings.Enabled` is false.
- [ ] 5.4 RED `service_test.go`: `queuePushScan` does not create a second
...
- [ ] 5.5 GREEN: implement `queuePushScan(ctx context.Context, tenant,
...
- [ ] 5.6 GREEN: add `go s.queuePushScan(context.Background(), s.tenant(ctx),
...
- [ ] 5.7 RED `service_test.go`: `PublishManifest` returns 201/success without
      blocking on scan completion (assert the call returns before any scan
      state transition is observable).
- [ ] 5.8 Confirm 5.2–5.4, 5.7 GREEN: `go test ./internal/app/regixtry/... -run
      'QueuePushScan|PublishManifest'`.

## Phase 6: Admin HTTP GET/PUT (Decision 5a) — depends on Phase 1

- [ ] 6.1 RED `internal/protocol/http/admin_handlers_test.go`: `GET
      /admin/v1/scan-policy` returns the current settings JSON; requires
      `requireAdminPrincipal`.
- [ ] 6.2 RED `admin_handlers_test.go`: `PUT /admin/v1/scan-policy` with
      `{"enabled":true,"severity_threshold":"critical_high"}` persists and
      round-trips.
- [ ] 6.3 RED `admin_handlers_test.go`: `PUT /admin/v1/scan-policy` with an
...
- [ ] 7.1 RED `internal/protocol/http/router_test.go`: `GET
      ...
      unscanned).
- [ ] 7.2 RED `router_test.go`: **route-collision case** — a manifest with tag
...
- [ ] 7.3 RED `router_test.go`: a caller with valid pull-only credentials for
...
- [ ] 7.4 GREEN: add `handleManifestScanStatus` to `router.go`, authorized via
...
- [ ] 7.5 Confirm 7.1–7.3 GREEN: `go test ./internal/protocol/http/... -run
      ManifestScanStatus`.

## Phase 8: TUI Policy Modal (Decision 6a) — depends on Phase 1 & 6

- [ ] 8.1 RED `internal/tui/session_test.go`: `scanPolicyModal.Active()`
      behaves like `trivyConfigModal.Active()` (open/closed state, focus
      cycling between the 2 fields).
- [ ] 8.2 RED `internal/tui/admin_views_test.go`: `renderScanPolicyModal` <= 13
...
- [ ] 8.3 RED `admin_views_test.go`: `renderScanPolicyModal` never renders
...
- [ ] 8.4 GREEN: add `scanPolicyModal` struct (own struct, sibling of
...
- [ ] 8.5 GREEN: add `renderScanPolicyModal` to `internal/tui/admin_views.go`
...
- [ ] 8.6 Live-render verification: throwaway debug test rendering
...
- [ ] 9.4 GREEN: compose the policy badge onto `renderTrivyTabs`'s existing
...
- [ ] 9.5 Live-render verification: throwaway debug test rendering the
...
- [ ] 9.6 Confirm 9.1–9.3 GREEN: `go test ./internal/tui/... -run
...
- [ ] 10.3 Combined live-render pass: one throwaway debug test rendering the
...
- [ ] 10.4 Resolve design.md's Open Questions: confirm at 150x24 the tab line
...
- [ ] 10.5 Update this file's checkboxes as work lands; save
